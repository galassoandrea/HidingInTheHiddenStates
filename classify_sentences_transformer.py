...
from ACDC.ACDC import ACDCEdge
...
for layer_num_from_end in layer_num_list:
    ...
    # Load embedding dataset for training and test and embedding dataset for circuit discovery (this includes corrupted statements)
    datasets = []
    circuit_datasets = []
    for dataset_name in dataset_names:
        datasets.append(pd.read_csv('embeddings/embeddings_with_labels_'+dataset_name+'_'+model_name.split("/")[-1]+'-all_'+str(abs(layer_num_from_end))+'_rmv_period.csv'))
        # The circuit discovery dataset does not include the "facts" dataset, since it is non-trivial to generate corrupted statements for it
        if dataset_name != "facts":
            circuit_datasets.append(pd.read_csv('embeddings/embeddings_with_labels_'+dataset_name+'_'+model_name.split("/")[-1]+'-clean-corrupted_'+str(abs(layer_num_from_end))+'_rmv_period.csv'))
    results = []
    for ds in range(len(dataset_names)):
        test_df = datasets[ds][:100]
        dfs_to_concatenate = datasets[:ds] + datasets[ds + 1:]
        train_df = pd.concat(dfs_to_concatenate, ignore_index=True)
        circuit_df = pd.concat(circuit_datasets, ignore_index=True)
        ...
        for i in range(repeat_each):
            ...
            # Run ACDC to perform circuit discovery on the trained transformer probe using the test data
            algorithm = ACDCEdge(model=model.model, model_name="TransformerProbe", embedding_dataset=circuit_df)
            circuit = algorithm.run()

            # Evaluate circuit on test data
            algorithm.evaluate_circuit(test_data=(test_embeddings, test_labels))
